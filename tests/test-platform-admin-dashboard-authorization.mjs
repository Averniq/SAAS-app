...
const paymentCutover = readFileSync(new URL('../remediation/payment-writer-cutover-candidate.sql', import.meta.url), 'utf8');
...
for (const fn of ['update_restaurant_order_status', 'issue_public_qr_table_token', 'get_public_qr_table_token_metadata']) {
  assert.match(migration, new RegExp(`function public\\.${fn}`, 'i'), `${fn} must retain an explicit platform branch`);
}
assert.match(paymentCutover, /revoke all on function public\.record_restaurant_order_payment\(uuid,uuid,text\) from public, anon, authenticated/i, 'the final convergence state must revoke the legacy Front Desk payment RPC again');
assert.doesNotMatch(paymentCutover, /grant execute on function public\.record_restaurant_order_payment/i, 'platform administrators must not receive payment access through the legacy writer');
